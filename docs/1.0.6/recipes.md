...
Left/right strips use a rotated full `Header` by default. Disable globally or per region:
...
|-------------------------------|----------|
| `ChromeIconForegroundBrush` | Default add / close / more icon stroke |
...
C# ghost controls resolve the same keys at runtime via `Application.TryGetResource` (see `DockThemeResources`). Drop-hint fallbacks (when keys are missing) use `SystemAccentColor` at reduced opacity.
...
// Restore without targeting a region:
dockShell.Collapse();
```
