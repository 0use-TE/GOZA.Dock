...
## Tab 条扩展 chrome（位置菜单与尾部工具）

可选 **⋮** 菜单与尾部内容插槽（Demo：中上区域）：
...
| 属性 | 作用 |
|------|------|
| `ShowTabStripPlacementPicker` | **⋮** 菜单 → 设置 `TabStripPlacement`（上 / 右 / 下 / 左） |
| `TabStripTrailingContent` | Add 与 ⋮ 之后的自定义 UI |
| `DockShell.DefaultTabStripPlacement` | 区域 `TabStripPlacement` 为 `null` 时的默认值 |

当 `ShowAddDoc`、`ShowTabStripPlacementPicker` 或 `TabStripTrailingContent` 任一为真时，即使 `ItemsSource` 为空也会保留 Tab 条头部。

## 侧栏竖排 Tab 标题

左/右 Tab 条默认将完整 `Header` 旋转 90° 显示。全局或单区域关闭：
...
|---------------------------|------|
| `ChromeIconForegroundBrush` | 默认添加 / 关闭 / 更多图标描边 |
...
C# 侧 ghost 控件通过 `Application.TryGetResource` 解析相同键名（见 `DockThemeResources`）。未覆盖拖放提示键时，回退色来自 `SystemAccentColor`（降低透明度）。
