...
- `ILayoutExpansionHost.Collapse()` / `DockShell.Collapse()` — 退出布局全屏展开，无需指定区域（未展开时为 no-op）。

### 集合变更重入修复

- `DockRegion` 将 `SelectedItem` 更新与默认选中推迟到 UI 线程（`DispatcherPriority.Background`），避免 Avalonia 集合变更处理器重入抛错。

### Chrome 与侧栏标题

- 默认 chrome 图标（添加 / 关闭 / 更多）使用 `DockChromeIconForegroundBrush`（`DockThemeResources.ChromeIconForegroundBrush`），不再绑定按钮 `Foreground`。
- 左/右竖排 Tab 标题改为整段 `Header` 旋转 90°，不再逐字堆叠。
- 关闭按钮与 `DockSplitter` 分隔条命中区域加大；Tab 条 chrome 尺寸针对 Semi 等主题微调。

### 拖放提示强调色

- 未覆盖拖放提示画刷时，回退色从 `SystemAccentColor` 派生（约 20% / 40% 透明度），不再使用固定蓝色。
