...
GOZA.Dock 2.0 是一个面向 AOT 的 Avalonia 标签页工作区。布局完全交由普通 Avalonia XAML 描述：你写一个 `Grid`，在每个单元格放一个 `DockRegion`，用 `DockSplitter` 隔开即可。**不**包含浮动窗口、递归停靠树、运行时 AXAML 加载，也不需要反射创建 View。
...
Dock Chrome 完全自带主题——`TabStrip`、`TabStripItem`、`Button`、`ContentControl` 等出现在 Dock 视觉树里的 Avalonia 内置控件都被显式覆盖为私有 `ControlTheme`，不会回退到你应用的主题。Fluent / Semi 等主题仅用于你自己的应用控件和 Tab 内容。该 `StyleInclude` 是编译型 AXAML，NativeAOT 与剪裁友好。
