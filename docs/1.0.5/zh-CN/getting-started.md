...
> **NuGet [1.0.5](https://www.nuget.org/packages/GOZA.Dock/1.0.5)** — 本节为 **1.0.5** 文档。顶栏 **Version** 可切换旧版本。API 参考（顶栏）来自最新源码构建。
...
`DockRegion` 在 `SelectedItem` 未设置时会**自动选中第一项**，内容区即可显示，无需额外绑定。需要布局恢复或显式选中时，再绑定 `SelectedItem`（见 Demo）。
