...
Practical patterns on top of the [API Reference](api-reference.md). Every snippet targets GOZA.Dock 3.0.8 / Avalonia 12.
...
Cached surfaces remain in the visual tree under the parking panel, so `AttachedToVisualTree` / `Loaded` do not mean the tab is showing. Implement [`IDockSurfaceLifecycle`](api-reference.md#idocksurfacelifecycle) on the view to pause and resume:
...
`OnDockSurfaceActivated` runs after the surface is parented into `PART_ContentHost`; `OnDockSurfaceDeactivated` runs on park and on `Evict`.
...
Or restyle the container instead of the content by supplying `TabItemTheme` (a `ControlTheme` for `TabStripItem`) — see [Theming](theming.md).
...
| Nothing renders / no dock chrome | `DockShell` not in tree / theme broken | Ensure a `DockShell` hosts regions (`DockShellStyles` loads with the shell) |
...
| Cached `WebView` / media keeps running while parked | The control stays attached under the parking panel | Implement `IDockSurfaceLifecycle` and pause in `OnDockSurfaceDeactivated` |
