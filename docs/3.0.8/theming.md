...
Vertical classic tabs keep the 32px strip metric, center the compact rotated title-and-close group with the same title/action spacing as horizontal tabs, add no spacer lanes, and pin Add → placement → maximize controls to the bottom edge. Their drag ghost keeps the live orientation and footprint by rotating an already-arranged header frame at render time:
