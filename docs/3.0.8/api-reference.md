...
Everything public in GOZA.Dock 3.0.8, with usage examples. The generated member-level reference is reachable from the navbar's **API Reference** entry.

> **Naming note.** 3.0 has no `DockItem` control. A "dock item" is a **tab item**: your view model implements [`IDockTabItem`](#idocktabitem) and the library renders it with the lookless [`DockTabHeader`](#docktabheader) control inside a [`DockRegion`](#dockregion). Everything you would expect from a `DockItem` (title, closability, surface reuse) is on `IDockTabItem`.

| Type | Namespace | Kind | Role |
|---|---|---|---|
| [`DockShell`](#dockshell) | `GOZA.Dock.Controls` | `ContentControl` | Root of a workspace; owns the optional view cache and `ColorTheme` |
...
| [`IDockSurfaceLifecycle`](#idocksurfacelifecycle) | `GOZA.Dock` | interface | Optional activate/deactivate for cached tab surfaces |
...
| `VsCodeColorTheme` / `VsCodeThemeJson` | `GOZA.Dock` | types | Strong-typed VS Code theme + AOT-safe JSON loader |
| `VsCodeThemeColors` | `GOZA.Dock` | static class | Official workbench color ID constants |
| `DockColorThemeCatalog` | `GOZA.Dock` | static class | Built-in palettes via `Create` → assign `DockShell.ColorTheme` |
...
The workspace root. It is intentionally thin: it themes the background/padding, applies optional VS Code workbench colors via `ColorTheme`, and — when `EnableViewCache` is on — creates a [`DockViewHost`](#dockviewhost) parking lot inside your content root. It does **not** own layout topology: the `Grid` you put in `Content` is the layout.
...
| `CloseButtonDisplayMode` | `DockTabCloseButtonDisplayMode` | `Always` | Controls when closable tabs show their close button; use `SelectedOrPointerOver` for the compact legacy behavior. |
| `TabScrollBarVisibility` | `ScrollBarVisibility` | `Hidden` | Tab-strip scroll-bar visibility. `Hidden` still supports wheel/touchpad scrolling; use `Auto` to show a bar only on overflow. |
...
- Switching away from a tab with `ReuseSurface = true` parks the surface actually hosted in the region (tracked independently of the deferred `SelectedItem` notification).
...
```csharp
public enum DockTabCloseButtonDisplayMode
{
    Always,
    SelectedOrPointerOver
}
```

`Always` is the default. This setting only affects tabs whose `IsClosable` is `true`; non-closable tabs never show a close button.

```xml
<DockRegion CloseButtonDisplayMode="SelectedOrPointerOver"
            TabScrollBarVisibility="Auto" />
```
...
- `Activate` sets `DataContext = tab`, re-enables hit-testing, then calls [`IDockSurfaceLifecycle.OnDockSurfaceActivated`](#idocksurfacelifecycle) when the control implements it.
- `Release` and `Evict` call `OnDockSurfaceDeactivated` before detaching. Park disables hit-testing so a parked `WebView` cannot steal input.
- Tabs with `ReuseSurface = false` pass straight through: `DockRegion` assigns the freshly built surface as `ActiveContent` without calling `DockViewHost`, so lifecycle callbacks are not raised.
...
Optional lifecycle for the **control** created by a tab's `DataTemplate`, not for the view model. A cached surface may remain attached under the invisible parking panel, so `AttachedToVisualTree` / `Loaded` / `Unloaded` are not a reliable "this tab is showing" signal.

`DockViewHost` calls:

| Method | When |
|---|---|
| `OnDockSurfaceActivated` | After the surface is parented into `PART_ContentHost` |
| `OnDockSurfaceDeactivated` | Before the surface is parked or evicted |

Implement it on expensive views that must pause while parked:

```csharp
public sealed class BrowserView : UserControl, IDockSurfaceLifecycle
{
    public void OnDockSurfaceActivated() => webView.IsVisible = true;

    public void OnDockSurfaceDeactivated()
    {
        webView.IsVisible = false; // or pause playback / stop a render loop
    }
}
```

The interface is optional and only applies to cached (`ReuseSurface = true`) surfaces driven by `DockViewHost`. Views that do not implement it keep the existing park/re-parent behaviour.
...
Pointer gestures for one tab strip: click to select, drag to reorder, drag out to move across regions, long-press to start a drag on touch. Thresholds: **6 px** of movement, **450 ms** long-press. The drag ghost is styled with the `DockDragGhost*` resources. Vertical ghosts clone the live tab width/height and rotate a horizontally arranged header with `RenderTransform`, so measure/arrange are not distorted by the rotation.
...
## What 3.0 deliberately does not have
