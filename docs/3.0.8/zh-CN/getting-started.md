...
> 当 `ReuseSurface = true` 时，`Id` 必须在整个 App 内稳定唯一——它是 Parking Lot 的缓存键。缓存表面仍挂在隐藏 Parking Lot 面板下；若暂停/恢复必须跟随当前 Tab 而不是可视树附加，请在 View 上实现 [`IDockSurfaceLifecycle`](api-reference.md#idocksurfacelifecycle)。
