...
GOZA.Dock 3.0.8 所有公开类型，含用法示例。生成的方法级索引可以从导航栏的 **API Reference** 入口访问。
...
| [`IDockSurfaceLifecycle`](#idocksurfacelifecycle) | `GOZA.Dock` | interface | 缓存 Tab 表面的可选激活/停用回调 |
...
| `CloseButtonDisplayMode` | `DockTabCloseButtonDisplayMode` | `Always` | 控制可关闭 Tab 的关闭按钮何时显示；设为 `SelectedOrPointerOver` 可恢复紧凑的选中/悬停显示方式。 |
| `TabScrollBarVisibility` | `ScrollBarVisibility` | `Hidden` | Tab 条滚动条可见性。`Hidden` 仍支持滚轮/触控板滚动；设为 `Auto` 仅在溢出时显示滚动条。 |
...
- 切走 `ReuseSurface = true` 的 Tab 时，Park 的是 Region 当前实际承载的表面（与延迟的 `SelectedItem` 通知分开跟踪）。
...
`Always` 是默认值。该设置只影响 `IsClosable = true` 的 Tab；不可关闭 Tab 始终不显示关闭按钮。
...
- `Activate` 会设置 `DataContext = tab`、启用命中测试，然后在控件实现 [`IDockSurfaceLifecycle`](#idocksurfacelifecycle) 时调用 `OnDockSurfaceActivated`。
- `Release` 与 `Evict` 会在摘下之前调用 `OnDockSurfaceDeactivated`。Park 会禁用命中测试，确保已 Park 的 `WebView` 不会偷走输入。
- `ReuseSurface = false` 的 Tab 走直通：`DockRegion` 把新构建的表面直接赋给 `ActiveContent`，不经过 `DockViewHost`，因此不会触发生命周期回调。
...
可选生命周期，实现在 Tab `DataTemplate` 创建的 **控件** 上，而不是 ViewModel。缓存表面可能仍挂在隐藏 Parking Lot 面板下，所以 `AttachedToVisualTree` / `Loaded` / `Unloaded` 并不能可靠表示“当前 Tab 正在显示”。

`DockViewHost` 的调用时机：

| 方法 | 时机 |
|---|---|
| `OnDockSurfaceActivated` | 表面已挂到 `PART_ContentHost` 之后 |
| `OnDockSurfaceDeactivated` | 表面被 Park 或 Evict 之前 |

给需要在 Park 期间暂停的昂贵 View 实现它：
...
public sealed class BrowserView : UserControl, IDockSurfaceLifecycle
{
    ...
    {
        webView.IsVisible = false; // 或暂停播放 / 停止渲染循环
    }
}
```

接口是可选的，且只作用于由 `DockViewHost` 驱动的缓存表面（`ReuseSurface = true`）。未实现的 View 仍使用原有的 Park / 重新挂载行为。
...
单个 Tab 条的指针手势：点击选中、拖动重排、拖出跨区、触屏长按起拖。阈值：**6 px** 位移、**450 ms** 长按。拖拽幽灵由 `DockDragGhost*` 资源系列样式化。垂直 Ghost 会克隆真实 Tab 的宽高，并用 `RenderTransform` 旋转已经水平排好的 Header，因此旋转不会回写到测量/布局。
