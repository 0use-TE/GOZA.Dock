...
[API 参考](api-reference.md)之上的常用模式。所有示例针对 GOZA.Dock 3.0.8 / Avalonia 12。
...
缓存表面仍挂在 Parking Lot 面板的可视树下，因此 `AttachedToVisualTree` / `Loaded` 并不表示当前 Tab 正在显示。在 View 上实现 [`IDockSurfaceLifecycle`](api-reference.md#idocksurfacelifecycle) 来暂停和恢复：
...
`OnDockSurfaceActivated` 在表面挂到 `PART_ContentHost` 之后触发；`OnDockSurfaceDeactivated` 在 Park 和 Evict 时触发。
...
或改容器外观而非内容——提供 `TabItemTheme`（针对 `TabStripItem` 的 `ControlTheme`），详见[主题](theming.md)：
...
| Dock Chrome 完全没渲染 | 没有 `DockShell` / 主题损坏 | Region 必须放在 `DockShell` 下（样式随 Shell 自动挂载） |
...
| 已 Park 的 `WebView` / 媒体仍在运行 | 控件仍挂在 Parking Lot 面板下 | 实现 `IDockSurfaceLifecycle`，在 `OnDockSurfaceDeactivated` 里暂停 |
