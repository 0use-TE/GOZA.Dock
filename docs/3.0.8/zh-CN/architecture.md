...
拖拽幽灵是 `DockDragGhost*` 键样式的 `Border`；Drop Hint 使用 `DockDropHint*`。垂直 Ghost 会在渲染时旋转已经排好的 Header 框架，从而保持真实 Tab 尺寸。
...
切走时控件被摘下，Park 进隐藏零像素 `Panel`（命中测试关闭）。若已 Park 的控件实现了 [`IDockSurfaceLifecycle`](api-reference.md#idocksurfacelifecycle)，`DockViewHost` 会调用 `OnDockSurfaceDeactivated`——可视树附加并不能可靠表示“正在显示”。移除可复用 Tab 时务必配对 `DockRegion.EvictView(tab)` 并 `Dispose` 你自己持有的非托管资源。
