...
The drag ghost is a `Border` styled from the `DockDragGhost*` resource keys; the drop hint uses `DockDropHint*`. Vertical ghosts keep the live tab footprint by rotating an already-arranged header frame at render time.
...
On deselection the surface is detached and parked in a hidden zero-pixel `Panel` (hit-testing off). `DockViewHost` then calls [`IDockSurfaceLifecycle.OnDockSurfaceDeactivated`](api-reference.md#idocksurfacelifecycle) when the parked control implements it — visual-tree attachment is not a reliable "visible" signal. Removing a reusable tab must be paired with `DockRegion.EvictView(tab)` and any disposal you own.
