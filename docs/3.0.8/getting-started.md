...
GOZA.Dock 3.0 is an AOT-first Avalonia tab workspace. Layout stays in ordinary Avalonia XAML: you author a `Grid`, place a `DockRegion` in each cell, and separate them with `DockSplitter`. There is no floating window, no recursive dock tree, no runtime AXAML loading, and no reflection-based view resolution.
...
```bash
dotnet add package GOZA.Dock --version 3.0.8
```
...
## 2. Dock chrome styles

Each `DockShell` loads `DockShellStyles.axaml` into its own `Styles` collection — you do **not** need an App-level `StyleInclude`.

```xml
<Application.Styles>
  <FluentTheme /> <!-- optional: your app controls / tab content -->
</Application.Styles>
```

Dock chrome is fully self-themed inside that shell (`TabStrip`, chrome buttons, splitter, …). Fluent / Semi only theme your own controls and tab content. Assign colors with `DockShell.ColorTheme`.
...
> `Id` must be stable and unique across the app when `ReuseSurface` is `true` — it is the parking-lot cache key. Cached surfaces stay attached under a hidden parking panel; implement [`IDockSurfaceLifecycle`](api-reference.md#idocksurfacelifecycle) on the view when pause/resume must follow the active tab rather than visual-tree attachment.
...
- [Migration from 2.0.x](migration.md).
