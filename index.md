---
_layout: landing
---

# GOZA.Dock

An AOT-first Avalonia tab workspace — fixed regions, draggable tabs, lookless controls, and optional view reuse.

```bash
dotnet run --project samples/GOZA.Dock.Demo.Desktop
dotnet add package GOZA.Dock --version 3.0.8
```

Use **Version** (package release) and **Lang** in the top navbar.

| Package | Docs |
|---------|------|
| **3.0.8** (latest) | [Quick Start](docs/3.0.8/getting-started.md) · [API Reference](docs/3.0.8/api-reference.md) · [Theming](docs/3.0.8/theming.md) · [Migration](docs/3.0.8/migration.md) |
| 3.0.0 (archived) | [Quick Start](docs/3.0.0/getting-started.md) · [API Reference](docs/3.0.0/api-reference.md) · [Theming](docs/3.0.0/theming.md) · [Migration](docs/3.0.0/migration.md) |
| 2.0.0 | [Quick Start](docs/2.0.0/getting-started.md) |
| 1.0.6 | [Quick Start](docs/1.0.6/getting-started.md) |
| 1.0.5 | [Quick Start](docs/1.0.5/getting-started.md) |
| 1.0.4 | [Quick Start](docs/1.0.4/getting-started.md) |
| 1.0.3 | [Quick Start](docs/1.0.3/getting-started.md) |
| 1.0.2 | [Quick Start](docs/1.0.2/getting-started.md) |
| 1.0.1 | [Quick Start](docs/1.0.1/getting-started.md) |
| 1.0.0 | [Quick Start](docs/1.0.0/getting-started.md) |

| | |
|---|---|
| Browser demo | [WASM demo](/GOZA.Dock/demo/) |
| API (latest build) | [API Reference](https://0use.net/GOZA.Dock/api/) |
| Repository | [GitHub](https://github.com/0use-TE/GOZA.Dock) |
